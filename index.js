...
const SUBCATEGORY_ORDER = ['N1', 'N2', 'N3', 'O1', 'O2', 'O3', 'O4'];
const CATEGORY_TOTAL_ORDER = ['C', 'E'];
const SUBCATEGORY_ORDER_INDEX = new Map(
    SUBCATEGORY_ORDER.map((category, index) => [category, index])
);
const DESKTOP_MAP_ZOOM_MARGIN = [120, 96, 120, 96];
const MOBILE_MAP_ZOOM_MARGIN = [140, 32, 104, 32];
const DESKTOP_BALLOON_MAX_WIDTH = 380;
const MOBILE_BALLOON_MAX_WIDTH = 310;
    ...
    fetch(DATA_URL)
        ...
        .then(obj => {
            ...
            obj.features.forEach(feature => {
                ...
                if (isGibddFeature(feature)) {
                    validFeatures.push(feature);
                    return;
                }

                const quantity = getFeatureQuantity(feature);
                const categories = sortSubcategories(getFeatureCategories(feature));

                if (quantity === null || categories.length === 0) return;
            ...
            allCategories = sortSubcategories(Array.from(categorySet));
...
function prepareFeatureBalloon(feature, quantity, categories) {
    ...
    const title = getFeatureTitle(feature) || properties.name || '';
    const rows = [];
    ...
    const categoryTotals = formatCategoryTotals(properties.categoryTotals || {});
    if (categoryTotals) {
        rows.push(createBalloonRow('Категории', categoryTotals, true));
    }

    const subcategoryTotals = formatSubcategoryTotals(properties.subcategoryCounts || {}, categories);
    if (subcategoryTotals) {
        rows.push(createBalloonRow('Подкатегории', subcategoryTotals, true));
    }

    if (!categoryTotals && quantity !== null) {
        rows.push(createBalloonRow(
            'Всего C/E',
            `<span class="map-balloon__category-total">${escapeHtml(quantity)}</span>`,
            true
        ));
    }
...
function formatCategoryTotals(totals) {
    const parts = CATEGORY_TOTAL_ORDER
        .filter(category => Number(totals[category] || 0) > 0)
        .map(category => (
            `<span class="map-balloon__category-total">${escapeHtml(category)} (${escapeHtml(totals[category])})</span>`
        ));

    return parts.join(' ');
}

function formatSubcategoryTotals(counts, categories) {
    const ordered = sortSubcategories(categories.length > 0 ? categories : Object.keys(counts));
    const parts = ordered
        .filter(category => Number(counts[category] || 0) > 0)
        .map(category => (
            `<span class="map-balloon__subcategory-total">${escapeHtml(category)} (${escapeHtml(counts[category])})</span>`
        ));
...
function getFeatureCategories(feature) {
    ...
    if (Array.isArray(feature.properties.categoryList)) {
        return feature.properties.categoryList.filter(category => SUBCATEGORY_ORDER_INDEX.has(category));
    }

    const raw = String(feature.properties.category || '').trim();
    if (!raw) return [];

    return raw
        .split(/[;,|]/)
        .map(item => item.trim())
        .filter(category => SUBCATEGORY_ORDER_INDEX.has(category))
        .filter((category, index, arr) => arr.indexOf(category) === index);
}

function sortSubcategories(categories) {
    return categories.sort((a, b) => {
        const ia = SUBCATEGORY_ORDER_INDEX.has(a) ? SUBCATEGORY_ORDER_INDEX.get(a) : Infinity;
        const ib = SUBCATEGORY_ORDER_INDEX.has(b) ? SUBCATEGORY_ORDER_INDEX.get(b) : Infinity;
...
function isCategoryFilterOff() {
    ...
    if (selectedCategories.size !== allCategories.length) return false;

    return allCategories.every(category => selectedCategories.has(category));
}
